...
describe "Uploadcare::Rails::Formtastic", type: :helper  do
  ...
  it 'should create single uploader field' do
    ...
    expect(tag).to have_selector('input[role="uploadcare-uploader"][data-multiple="false"]', visible: :all)
  end

  it 'should create multiple uploader field' do
    tag = @form.input :file, as: :uploadcare_multiple_uploader
    expect(tag).to have_selector('input[role="uploadcare-uploader"][data-multiple="true"]', visible: :all)
  end
end
